...
int main() {
	...
	while(t--){
		...
		double sum=arr[0],count=0;
		for(ll i=1;i<n;i++){
			if((double)(arr[i]/sum) > (double)(k/(double)100)) {
				// cout<<arr[i]<<" "<<sum<<endl;
				// while((double)(arr[i]/sum) > (double)(k/(double)100)){
				// 	sum++;count++;
				// }
				ll rem = ((ll)arr[i]*100)%k;
				if(rem!=0) arr[i] += (k-rem);
				ll d = (arr[i]*100)/k - sum;
				count+=d;
				sum += d;
			}
			sum += arr[i];
